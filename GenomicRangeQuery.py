...
"""
# working from minimum value upwards 66% performance O(N+M)
def solution(S, P, Q):
    answers = []
    m = len(P)
    for i in range(m):
        search_range = S[P[i]:Q[i]+1]
        if 'A' in search_range:
            answers.append(1)
        elif 'C' in search_range:
            answers.append(2)
        elif 'G' in search_range:
            answers.append(3)
        else:
            answers.append(4)
    return answers
...
def solution(S, P, Q):
    answers = []    
    N = len(S)
    impact_factors = dna_to_impact_factors(S, N)
    ...
    for query in range(M):
        ...
        q = Q[query]
        if p == q:            
            answers.append(impact_factors[p])        
        else:        
            answers.append(min(impact_factors[p:q+1]))

    return answers

def dna_to_impact_factors(S, N):
    impact_factors = []
    for s in range(N):
        letter = S[s]
        if letter == 'A':
