...
cross only when leaves appear at every position across the river from 1 to X (that is, we want to find the earliest moment when all the positions from 1 to X are covered by leaves). You may assume that the speed of the current in the river is negligibly small, i.e. the leaves do not change their positions once they fall in the river.
...
def solution(X, A):
    ...
    positions_needed = set(range(1, X+1))
    positions_found = set()
    for i in range(N):
        current_position = A[i]
        if current_position in positions_needed:  # line is redundant if we change position_found
            positions_found.add(current_position) # contains position_needed
...
# 100% Correctness 54% Performance O(N^2)
def solution(X, A):
    N = len(A)
    ...
    positions_needed = list(range(1, X+1))
    for i in range(N):
        if A[i] in positions_needed:
            positions_needed.remove(A[i])        
